...
RUN find target/release -name "*elisam_dev*" -delete 2>/dev/null || true
RUN find target/release/deps -name "*elisam_dev*" -delete 2>/dev/null || true
...
COPY --from=website-builder /usr/src/app/target/release/elisam_dev ./elisam_dev
